...
object_detector = cv2.createBackgroundSubtractorMOG2(
    history=None, varThreshold=None)
# 100,5
...
while True:
    ...
    for box_id in boxes_ids:
        ...
        if (tracker.getsp(id) < tracker.limit()):
            cv2.putText(roi, str(id)+" "+str(tracker.getsp(id))+"km/h",
                        (x, y-15), cv2.FONT_HERSHEY_PLAIN, 1, (255, 255, 0), 2)
            cv2.rectangle(roi, (x, y), (x + w, y + h), (0, 255, 0), 3)
        else:
            cv2.putText(roi, str(id) + " "+str(tracker.getsp(id))+"km/h",
                        (x, y-15), cv2.FONT_HERSHEY_PLAIN, 1, (0, 0, 255), 2)
